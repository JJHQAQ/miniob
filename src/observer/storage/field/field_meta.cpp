...
const static Json::StaticString FIELD_CAN_NULL("can_null");

FieldMeta::FieldMeta() : attr_type_(AttrType::UNDEFINED), attr_offset_(-1), attr_len_(0), visible_(false),can_null_(false) {}

FieldMeta::FieldMeta(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible,bool can_null)
{
  [[maybe_unused]] RC rc = this->init(name, attr_type, attr_offset, attr_len, visible,can_null);
  ASSERT(rc == RC::SUCCESS, "failed to init field meta. rc=%s", strrc(rc));
}

RC FieldMeta::init(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible,bool can_null)
{
...
void FieldMeta::to_json(Json::Value &json_value) const
{
  ...
  json_value[FIELD_CAN_NULL] = can_null_;
}
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field)
{
  ...
  const Json::Value &can_null_value = json_value[FIELD_CAN_NULL];
  ...
  bool        can_null = can_null_value.asBool();
  return field.init(name, type, offset, len, visible,can_null);
}
